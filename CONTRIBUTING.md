...
- TDD: write the test first. Unit tests live in `test/`; integration tests (marked
  ...
  CI also resolves the exact Replicant 1.2.2 floor independently of the current
  1.2.2 lock. To reproduce that selector in an isolated instrument worktree:

  ```bash
  ASH_REPLICANT_REPLICANT_VERSION=1.2.2 \
    scripts/with-release-runtime.sh mix deps.unlock replicant
  ASH_REPLICANT_REPLICANT_VERSION=1.2.2 \
    scripts/with-release-runtime.sh mix deps.get
  ASH_REPLICANT_REPLICANT_VERSION=1.2.2 \
    scripts/with-release-runtime.sh \
      scripts/assert-dependency-version.sh replicant '== 1.2.2'
  ```
