...
| Batch re-delivery after a crash between commit and ack | `:catching_up` through the replay | The re-delivered committed batch is a whole-batch skip — zero new effects, watermark unchanged | None | One batch, one effect — the skip is red-capability-proven against its guard |
...
| Standalone message re-delivery (crash between effect and ack) | `:catching_up` through the replay | The claim replays — zero net effect (idempotency, never a one-time nonce) | None | The claim is the dedup; a lawful replay is a durable no-op |
| External-peer message ambiguity (crash mid-effect) | `:catching_up` until resolved | AshOnetime three-state recovery: re-delivery runs the recover path and finalizes ONCE; the watermark advances only after finalized/replayed success | Resolve the peer's in-flight operation if it asks; otherwise none | An ambiguous external effect neither duplicates nor silently drops |
| Message claim past its retention window | At-risk telemetry while retained; re-execution after the reaper | A claim that died at `retain_until` re-executes on re-delivery — the operator-sized replay window | Size `recovery_horizon` ≥ the reclamation cadence (activation refuses mismatches) | The horizon alerts BEFORE recovery becomes impossible (ADR-0022) |
