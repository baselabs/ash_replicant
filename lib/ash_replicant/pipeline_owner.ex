defmodule AshReplicant.PipelineOwner do
  ...
  the connection; it observes process death and cleans up:
    ...
    * the owner exits (host-tree shutdown, a linked caller's crash) — the
      trapped `:EXIT` handler stops the pipeline first, then the same
      cleanup runs. A linked caller's `:normal` exit is ignored so a bare
      `AshReplicant.start_link/1` caller finishing does not stop the pipeline.
  ...
  (cast) and links it. A caller that dies mid-handshake leaves either no
  generation (activation had not finished) or a generation whose owner is
  still pending — the pending owner watches the caller and reaps its own
  ...
  @doc """
  Start the owner for one slot's pipeline (linked to the caller on
  success). This is the body of `AshReplicant.start_link/1`; hosts should
  prefer the `child_spec/1` under their supervisor. Activation runs in the
  caller and returns the activation chain's own error shapes synchronously
  (`:sink_required`, `:slot_already_active`, …).
  """
  ...
  # A linked caller finishing normally (e.g. the Task that raced a start)
  # does not own the pipeline's lifetime — ignore. Post-adoption this also
  # swallows the pending caller-monitor's late :DOWN.
  def handle_info({:EXIT, _pid, :normal}, state), do: {:noreply, state}

  # Host-tree shutdown or a linked caller's crash: stop the pipeline first,
